  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Correlación Térmica", DTOutput("tabla_cor"), plotlyOutput("plot_cor"), uiOutput("texto_cor")),
...
server <- function(input, output, session) {
  ...
  output$texto_criticos <- renderUI({
    ...
    HTML(glue("<p><strong>Equipo más crítico:</strong> {eq$Equipo} <br>
               ...
               <strong>% de registros bajo 4°C:</strong> {round(eq$pct_bajo, 1)}%</p>"))
  })
  
  output$texto_mant <- renderUI({
    req(resultados$mant)
    HTML(glue("<p><strong>Intervenciones registradas:</strong> {nrow(resultados$mant)} <br>
  ...
  output$texto_modelo_extra <- renderUI({
    ...
    HTML(glue("
      ...
      <ul>
        <li><b>Residuos vs Ajustados:</b> No se observan patrones sistemáticos, indicando homocedasticidad.</li>
        <li><b>QQ Plot:</b> Los residuos se alinean razonablemente con la normalidad.</li>
        <li><b>Distancia de Cook:</b> Ningún punto supera el umbral crítico, no hay observaciones influyentes graves.</li>
      </ul>
    "))
  })
  ...
  output$texto_graficos <- renderUI({
    ...
    HTML(glue("
      <p><b>Visualización:</b> {bajo_4} de {total} registros ({pct_bajo}%) presentan Δ Agua < 4°C.</p>
      <p>Equipos más comprometidos:</p>
      <ul>
